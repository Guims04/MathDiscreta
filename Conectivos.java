...
public class Conectivos extends TabelaVerdadeDois{
	...
	public static boolean[][] BiCondicional(int coluna1, int coluna2) {
		...
		for (int row = 0; row < numRow; row++) {
			for (int column = 0; column < 1; column++) {
				result[row][column] = (tabelaVerdade[row][coluna1] && tabelaVerdade[row][coluna2]) || (!tabelaVerdade[row][coluna1] && !tabelaVerdade[row][coluna2]);
			}
		}
		return result;
	}
	public static boolean[][] BiCondicionalOperação(boolean[][] result1, boolean[][] result2) {
		...
		for (int row = 0; row < numRow; row++) {
			for (int column = 0; column < 1; column++) {
				result[row][column] = (result1[row][column] && result2[row][column]) || (!result1[row][column] && !result2[row][column]);
			}
		}
		return result;
	}
	public static boolean[][] BiCondicionalOperaçãoDois(boolean[][] res, int coluna) {
		...
		for (int row = 0; row < numRow; row++) {
			for (int column = 0; column < 1; column++) {
				result[row][column] = (res[row][column] && tabelaVerdade[row][coluna]) || (!res[row][column] && !tabelaVerdade[row][coluna]);
			}
		}
		return result;	
	}
